...
DEFINE CLASS VFPNode AS IConnectionNode OF TreeNodes.prg
	...
	PROCEDURE OnInit()
      ...
      * RAS 29-Jul-2006, Added option to store column sorting separate from the other objects
      THIS.CreateOption("SortColumns", .F.)
	...
	FUNCTION GetConnection(cDatabase)
         ...
         * RAS 29-Jul-2006, Added option to store column sorting separate from the other objects
         oConn.SortColumns = THIS.FindOption("SortColumns", .F.)
		ENDIF
...
DEFINE CLASS VFPDatabaseNode AS VFPNode
	...
	* Get the database to connect to
	...
	PROCEDURE OnShowProperties()
		...
		DO FORM VFPProperties WITH THIS TO m.lSuccess
      
		IF VARTYPE(lSuccess) == 'L' AND m.lSuccess
			THIS.RefreshNode()
		ENDIF
	ENDPROC
...
DEFINE CLASS VFPTableNode AS VFPNode
	...
	* Browse VFP Data	
	...
	FUNCTION OnDefaultQuery()
		*< RAS 22-Feb-2009, Change because of the new FoxPro Table Directory connection
      *< RETURN "SELECT * FROM " + THIS.FixName(THIS.NodeData.Name)
      LOCAL lcTableName
      
      IF TYPE("this.NodeData.Name") == 'C'
         * Regular FoxPro table connection
         lcTableName = "SELECT * FROM " + this.FixName(THIS.NodeData.Name)
      ELSE
         * FoxPro Table Directory connection
         lcTableName = "SELECT * FROM " + this.FixName(this.NodeText)
      ENDIF
...
* RAS 22-Feb-2009, created new connection type
*DEFINE CLASS VFPTableDirectoryNode AS VFPNode
DEFINE CLASS VFPTableDirectoryNode AS VFPDirectoryNode
   ImageKey = "microsoft.imagedatabases"
   NodeText = NODETEXT_VFPDATABASES_LOC
   ...
   *< RAS 22-Feb-2009, key code is in parent class. No need to run it twice
   *< PROCEDURE OnInit()
   *<    DODEFAULT()
   *< 
   *<    * all CreateOption values are persisted between sessions
   ...
   * Get the source directory before to display all tables
   *< FUNCTION OnFirstConnect()
   *<    LOCAL cSourceDir
   *<    LOCAL lSuccess
   *< 
   ...
   * populate with all DBC files in selected directory
   FUNCTION OnPopulate()
      ...
      cSourceDirectory = THIS.FindOption("SourceDirectory", '')
      IF !EMPTY(cSourceDirectory) AND DIRECTORY(cSourceDirectory)
         nCnt = ADIR(aFileList, ADDBS(cSourceDirectory)  + "*.dbf")
         FOR i = 1 TO nCnt
            oChildNode = THIS.CreateNode("VFPTableNode", '', LOWER(ADDBS(cSourceDirectory) + aFileList[i, 1]), JUSTSTEM(aFileList[i, 1]))
            oChildNode.SetOption("TableName", ADDBS(cSourceDirectory) + aFileList[i, 1])
            THIS.AddNode(oChildNode)
         ENDFOR
      ENDIF
   ENDFUNC
   ...
   FUNCTION OnDefaultQuery()
      ...
      *< SET STEP ON 
      ...
      ELSE
         * parent node is a Table Connection, so must evaluate differently
         lcTableName = "SELECT * FROM " + THIS.FixName(FORCEEXT(THIS.FindOption("TableName", ''), ''))
      ENDIF
