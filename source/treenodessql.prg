...
DEFINE CLASS SQLNode AS IConnectionNode OF TreeNodes.prg
	...
	*UserPassword = '' && don't create an Option for this because we don't want it persisted
	...
	FUNCTION GetConnection(cServer, cDatabase)
         ...
         * RAS 29-Jul-2006, Added option to address column sorting separate from the other objects
         oConn.SortColumns = THIS.GetOption("SortColumns", .F.)
		ENDIF
		...
		* use to get available SQL servers
		...
		CATCH TO oException
			IF VARTYPE(oException.UserValue) == 'C'
				MESSAGEBOX(oException.UserValue, 0+48, DATAEXPLORER_LOC + " - " + LOWER(m.oException.Procedure))
			ENDIF
		ENDTRY
